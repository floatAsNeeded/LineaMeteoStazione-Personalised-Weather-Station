...
#include "epaper_fonts.h"       //https://github.com/G6EJD/ESP32-e-Paper-Weather-Display
#include "lang.h"
...
void BeginSleep() {
  //display.powerOff();
  if (timeClient.getHours() >= 1 && timeClient.getHours() < 6)
  {
    SleepDuration = 7200; //2 hour at night
  }
  else
  {
...
void airQuality() {
  ...
  Firebase.setInt(Weather, "Inside/AirQuality", AQIndex);
}
...
String CalculateIAQ(float score) {
  ...
  else
  {
    ...
    else if (score >= 100 && score < 150 ) IAQ_text += "Unhealthy for Sensitive";
...
void getValues()
{
  ...
  {
    VOLTAGEOUT = Weather.to<float>();
  }
  if (Firebase.getInt(Weather, "/Battery/CellPercentage"))
  {
    PERCENTAGEOUT = Weather.to<int>();
  }
  ...
  if (Firebase.getString(Weather, "/Display/FastRefresh"))
  {
    FASTREFRESH = Weather.to<const char *>();
  }
  //config.rtdb.data_type_stricted = true;
  ...
  if (Firebase.getFloat(Weather, "/Light/SolarRadiation"))
  {
    SolarRadiation = Weather.to<float>();
  }
  config.rtdb.data_type_stricted = false;
  bme.performReading();
  //pressure = bme.pressure + CALIBRATION;
  //pressurehpa = pressure / 100;
  humidityIN = bme.humidity;
  temperatureIN = (bme.temperature * constantF) + constantF32;
  //Firebase.setFloat(Weather, "Pressure/Pressure", pressurehpa);
  Firebase.setFloat(Weather, "Inside/Temperature", bme.temperature);
...
void maxminvalues()
{
  ...
  {
    maxTemp = (Weather.to<float>() * constantF) + constantF32;
  }

  if (Firebase.getFloat(Weather, "/SHT3x/Temperature/TemperatureMin"))
  {
    minTemp = (Weather.to<float>() * constantF) + constantF32;
  }
...
void DrawMainWeatherSection(int x, int y) {
  ...
  if (millis() - checkopenweather >= 600000 || justrestart == true)
  {
    int i;
    ...
    justrestart = false;
    checkopenweather = millis();
    ow.getForecast(current, hourly, daily, api_key, latitude, longitude, units, language);
    DescriptionDisplay = hourly->description[i];
    CloudsDisplay = current->clouds;
    VisibilityDisplay = current->visibility;
    DateTime nowSunrise = current->sunrise;
    SunriseHour = nowSunrise.hour() + TIMEZONE;
    SunriseMinute = nowSunrise.minute();
    DateTime nowSunset = current->sunset;
    SunsetHour = nowSunset.hour() + TIMEZONE;
    SunsetMinute = nowSunset.minute();
    ...
    //{
    drawStringMaxWidth(x - 170, y + 90, 28, DescriptionDisplay , LEFT);
    Visibility(275, 265, String(VisibilityDisplay) + "m");
    CloudCover(355, 265, CloudsDisplay);
  ...
  u8g2Fonts.setFont(u8g2_font_helvB10_tf);
  drawString(x - 167, y + 117,  UVINDEX + String(UVindex, 1), LEFT);
  drawString(x - 167, y + 137,  SOLARRADIATION + String(SolarRadiation, 1) + " W/m2", LEFT);

  if (language == "it")
  {
    if (UVindex < 3)
    {
      drawString(x - 82, y + 117,  + "    BASSO", LEFT);
    }
    if (UVindex >= 3 && UVindex < 6)
    {
      drawString(x - 77, y + 117,  + "    MEDIO", LEFT);
    }

    if (UVindex >= 6 && UVindex < 9)
    {
      drawString(x - 82, y + 117,  + "    ALTO", LEFT);
    }

    if (UVindex >= 9)
    {
      drawString(x - 77, y + 117,  + "     ESTREMO", LEFT);
    }
  }
  else
  {
    if (UVindex < 3)
    {
      drawString(x - 82, y + 117,  + "  LOW", LEFT);
    }
    if (UVindex >= 3 && UVindex < 6)
    {
      drawString(x - 77, y + 117,  + "  MEDIUM", LEFT);
    }

    if (UVindex >= 6 && UVindex < 9)
    {
      drawString(x - 82, y + 117,  + "  HIGH", LEFT);
    }

    if (UVindex >= 9)
    {
      drawString(x - 77, y + 117,  + "  EXTREME", LEFT);
    }
  }
  if (language == "it")
  {
    drawString(x - 167, y + 157,  + "IQA  " + String(AQIndex), LEFT);
  }
  else
  {
    drawString(x - 167, y + 157,  + "AQI  " + String(AQIndex), LEFT);
  }
  drawString(x - 110, y + 157, AQI, LEFT);
  drawString(x - 167, y + 177,  DEWPOINT + String((dewPoint * constantF) + constantF32, 1) + SymbolTemperature, LEFT);
  drawString(x - 167, y + 197,  + "WindChill    " + String((windchill * constantF) + constantF32, 1) + SymbolTemperature, LEFT);
  drawString(x - 167, y + 217, HEATINDEX + String((heatIndex * constantF) + constantF32, 1) + SymbolTemperature , LEFT);

  if (language == "it")
  {
    if (heatIndex > 27 && heatIndex <= 32)
    {
      drawString(x - 10, y + 217,  + "  Cautela", LEFT);
    }
    if (heatIndex > 32 && heatIndex <= 39)
    {
      drawString(x - 10, y + 217,  + "  Estrema Cautela", LEFT);
    }

    if (heatIndex > 39 && heatIndex <= 51)
    {
      drawString(x - 10, y + 217,  + "  Pericolo", LEFT);
    }

    if (heatIndex > 51)
    {
      drawString(x - 10, y + 217,  + "  Estremo Pericolo", LEFT);
    }
  }
  else
  {
    if (heatIndex > 27 && heatIndex <= 32)
    {
      drawString(x - 40, y + 217,  + " Caution", LEFT);
    }
    if (heatIndex > 32 && heatIndex <= 39)
    {
      drawString(x - 40, y + 217,  + " Extreme Caution", LEFT);
    }

    if (heatIndex > 39 && heatIndex <= 51)
    {
      drawString(x - 40, y + 217,  + "  Danger", LEFT);
    }

    if (heatIndex > 51)
    {
      drawString(x - 40, y + 217,  + "  Extreme Danger", LEFT);
    }
  }
  ...
  CloudCover(355, 265, CloudsDisplay);
  u8g2Fonts.setFont(u8g2_font_helvB08_tf);
  drawString(305, 275, SUNRISE, LEFT);
  drawString(350, 275, SUNSET, LEFT);
  ...
  drawString(305, 290, leadingzeroHour + String(SunriseHour) + ":" + leadingzeroMinute + String(SunriseMinute), LEFT);
  ...
  drawString(350, 290, leadingzeroHour + String(SunsetHour) + ":" + leadingzeroMinute + String(SunsetMinute), LEFT);
  DrawMainWx(x, y + 60);
  display.drawRect(0, y + 70, 225, 38, GxEPD_BLACK);
...
void DrawMainWx(int x, int y) {
  ...
  drawString(x + 9, y - 22, String((temp * constantF) + constantF32, 1) + SymbolTemperature, CENTER); // Show current Temperature
  u8g2Fonts.setFont(u8g2_font_helvB12_tf);
  drawString(x + 12, y - 3, String(maxTemp, 1) + "°| " + String(minTemp, 1) + "°", CENTER); // Show forecast high and Low
  u8g2Fonts.setFont(u8g2_font_helvB10_tf);
  ...
  u8g2Fonts.setFont(u8g2_font_helvB14_tf);
  drawString(x + 80, y + 30, INSIDE, LEFT);
  drawString(x + 80, y + 53, String(temperatureIN, 1) + SymbolTemperature + " | " + String(humidityIN) + "%", LEFT); // Show forecast high and Low

  if (language == "it")
  {
    drawString(x + 192, y + 53, "UR", LEFT);
  }
  else
  {
    drawString(x + 192, y + 53, "RH", LEFT);
  }
}
...
void DisplayDisplayWindSection(int x, int y, int Cradius) {
  ...
  drawString(x + 3, y - 3, String(WindSpeed / constantwind, 1) + (MeasurementUnitWind), CENTER);
}
//#########################################################################################//
...
void DrawPressureAndTrend(int x, int y) {
  drawString(x, y, String(pressurehpa / constantpressure, 1) + MeasurementUnitPressure, CENTER);
}
...
void DisplayPrecipitationSection(int x, int y) {
  ...
  u8g2Fonts.setFont(u8g2_font_helvB10_tf);
  drawString(x + 5, y - 5, String (RAIN) + String(mmPioggia / constantrain, 1)  +  MeasurementUnitRain, LEFT);
  drawString(x + 5, y + 15, String(RAINRATE) + String(rainrate / constantrain, 1)  + MeasurementUnitRain + "/h", LEFT);
  drawString(x + 5, y + 35, String(RAINRATEMAX) + String(rainrateMax / constantrain, 1) + MeasurementUnitRain + "/h", LEFT);
  drawString(x + 5, y + 55, String(GUST) + String(Gust / constantwind, 1) + (MeasurementUnitWind), LEFT);
}
...
void DrawAstronomySection(int x, int y) {
  u8g2Fonts.setFont(u8g2_font_helvB08_tf);
  display.drawRect(x + 15, y + 105, 150, 50, GxEPD_BLACK);
  const int day_utc   = timeClient.getDate();
  const int month_utc = timeClient.getMonth();
  const int year_utc  = timeClient.getYear();
  //Serial.println(month_utc);
  drawString(x + 120, y + 125, MoonPhase(day_utc, month_utc, year_utc), RIGHT);
  DrawMoon(x + 105, y + 90, day_utc, month_utc, year_utc, Hemisphere);
}
//#########################################################################################//
...
String MoonPhase(int d, int m, int y) {
  ...
  if (language == "it")
  {
    ...
    if (b == 7) return "Luna Nuova";              // New;              0%  illuminated
    if (b == 0) return "Luna Calante"; // Waxing crescent; 25%  illuminated
    if (b == 1) return "Ultimo Quarto";   // First quarter;   50%  illuminated
    if (b == 2) return "Gibbosa Calante";    // Waxing gibbous;  75%  illuminated
    if (b == 3) return "Luna Piena";             // Full;            100% illuminated
    if (b == 4) return "Gibbosa Crescente"; // Waning gibbous;  75%  illuminated
    if (b == 5) return "Primo Quarto";    // Third quarter;   50%  illuminated
    if (b == 6) return "Luna Crescente";  // Waning crescent; 25%  illuminated
    return "";
  }
  else
  {
    if (Hemisphere == "south" || Hemisphere == "north") b = 7 - b;
    if (b == 7) return TXT_MOON_NEW;              // New;              0%  illuminated
    if (b == 0) return TXT_MOON_WANING_CRESCENT;  // Waxing crescent; 25%  illuminated
    if (b == 1) return TXT_MOON_THIRD_QUARTER;    // First quarter;   50%  illuminated
    if (b == 2) return TXT_MOON_WANING_GIBBOUS;   // Waxing gibbous;  75%  illuminated
    if (b == 3) return TXT_MOON_FULL;             // Full;            100% illuminated
    if (b == 4) return TXT_MOON_WAXING_GIBBOUS;   // Waning gibbous;  75%  illuminated
    if (b == 5) return TXT_MOON_FIRST_QUARTER;    // Third quarter;   50%  illuminated
    if (b == 6) return TXT_MOON_WAXING_CRESCENT;  // Waning crescent; 25%  illuminated
    return "";
  }
}
...
void CloudCover(int x, int y, int CCover) {
  addcloud(x - 9, y - 3, VerySmall * 0.5, 2); // Cloud top left
  addcloud(x + 3, y - 3, VerySmall * 0.5, 2); // Cloud top right
  addcloud(x, y,         VerySmall * 0.5, 2); // Main cloud
  u8g2Fonts.setFont(u8g2_font_helvB08_tf);
...
void DrawBattery(int x, int y) {
  ...
  drawString(x + 61, y - 11, String(percentage) + " % ", RIGHT);
  drawString(x + 5, y - 11, "IN->", RIGHT);
...
void DrawBatteryOUTSIDE(int x, int y) {
  ...
  drawString(x + 288, y - 11, "OUT->", RIGHT);
  drawString(x + 380, y - 11, String(percentage) + " % ", RIGHT);
...
void setup() {
  ...
  if (Firebase.getInt(Weather, "/Connection/DEVICE3/ResetWiFi"))
  {
  ...
  Firebase.deleteNode(Weather, "/Connection/DEVICE3/UpdateHere");
  Firebase.setString(Weather, "/Connection/DEVICE3/Version", Version);

  if (Firebase.getString(Weather, "/Connection/DEVICE3/Update"))
  {
