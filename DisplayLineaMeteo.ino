...
String SOLARRADIATION = "SolarRadiation  ";
String HEATINDEX = "HeatIndex   ";
String UPDATE = "Last Updated On";
String INSIDE = "INDOOR";
String RAIN = "     Rain24H";
...
void getValues()
{
  ...
  {
    FASTREFRESH = Weather.stringData();
  }
  ...
  {
    SolarRadiation = Weather.floatData();
  }
  bme.performReading();
...
void maxminvalues()
{
  ...
  {
    maxTemp = (Weather.floatData() * constantF) + constantF32;
  }

  if (Firebase.getFloat(Weather, "SHT3x/Temperature/TemperatureMin"))
  {
    minTemp = (Weather.floatData() * constantF) + constantF32;
  }
...
void DrawMainWeatherSection(int x, int y) {
  ...
  if (millis() - checkopenweather >= 600000 || justrestart == true)
  {
    ...
    DescriptionDisplay = current->description;
...
void DrawMainWx(int x, int y) {
  ...
  u8g2Fonts.setFont(u8g2_font_helvB14_tf);
  drawString(x + 100, y + 30, INSIDE, LEFT);
...
void DisplayPrecipitationSection(int x, int y) {
  ...
  u8g2Fonts.setFont(u8g2_font_helvB10_tf);
  drawString(x + 5, y - 5, String(mmPioggia / constantrain, 1)  + MeasurementUnitRain + String (RAIN), LEFT);
  drawString(x + 5, y + 15, String(rainrate / constantrain, 1)  + MeasurementUnitRain + "/h" +  String(RAINRATE), LEFT);
  drawString(x + 5, y + 35, String(rainrateMax / constantrain, 1) + MeasurementUnitRain + "/h" + String(RAINRATEMAX), LEFT);
  drawString(x + 5, y + 55, String(Gust / constantwind, 1) + (MeasurementUnitWind) + String(GUST), LEFT);
}
